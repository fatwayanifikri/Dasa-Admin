<?php namespace App\Http\Controllers;
  ...
  use PDF;
	...
	class AdminProduksiSPKTapingController extends \crocodicstudio\crudbooster\controllers\CBController {
...
//---------------------------
 ...
 return view('viewindex/Project_SPK_Produksi/custom_spk_view_taping',$data);
...
/*--------------------FUNGSI UNTUK PAGE INPUT PROSES (DONE)-----------------*/
/*--------------------------------------------------------------------------*/


//--VIEW PAGE INPUT PROSES (DONE)--
...
//---------------------------
$query4 = \App\Models\t212_prosesproduksi::where('id_spk','=',0)
->where('status','=',1)
...
->get();
  ...
  return view('viewindex/Project_SPK_Produksi/custom_input_proses',$data);
	

}


/*-INPUT DATA BARU KE TABEL PROSES PRODUKSI-*/

public function input_proses_produksi(Request $request)
{
$datenow = Carbon::now();
$getJabatan=Crudbooster::myPrivilegeId();
$EmployeeID=Crudbooster::myId();
	...
	DB::table('t212_prosesproduksi')->insert([ 
		
		'id_spk' => 0,
		'proses_produksi' => $request->post('proses_produksi'),
		'hasil_good' => $request->post('hasil_good'),
		'nc' => $request->post('nc'),
		'ng' => $request->post('ng'),
		'nama_operator' => $request->post('nama_operator'),
		'mulai' => $request->post('mulai'),
		'selesai' => $request->post('selesai'),
		'paraf' => $request->post('paraf'),
		'keterangan_proses' => $request->post('keterangan_proses'),
		'status' => 1,
...
public function proses_done($id)
{
$datenow = Carbon::now();
$getJabatan=Crudbooster::myPrivilegeId();
$EmployeeID=Crudbooster::myId();
...
CRUDBooster::redirect(CRUDBooster::mainpath(),"Update Status Berhasil","success");
}

/*------------------END FUNGSI UNTUK PAGE INPUT PROSES (DONE)---------------*/
/*--------------------------------------------------------------------------*/
